...
public class GameApiService
{
    ...
    private const string BASE_URL = "https://68f95aaadeff18f212b95116.mockapi.io";
    ...
    public async Task<ItemJogador[]> GetItensJogador(int jogadorId)
    {
        try
        {
            string url = $"{BASE_URL}/Jogador/{jogadorId}/Item";
            Debug.Log($"GET: {url}");
    ...
    public async Task<ItemJogador> GetItem(string jogadorId, string itemId)
    {
        try
        {
            string url = $"{BASE_URL}/Jogador/{jogadorId}/Item/{itemId}";
            Debug.Log($"GET: {url}");
